...
public final class RedisUtils {
    ...
    public void del(String... keys) {
        if (keys != null && keys.length > 0) {
            if (keys.length == 1) {
                ...
                Boolean count = redisTemplate.delete(String.valueOf(keySet));
                log.debug("----------------------------------------");
                log.debug("成功删除缓存："+ keySet.toString());
                log.debug("删除缓存个数："+ count + "个");
                log.debug("----------------------------------------");
            }
    ...
    /**
     * 批量或单个删除带前缀的key  例如Spring：com：abc：Student
     * @param prefix
     * @param ids
     */
     ...
     * redis管道操作，批量插入key
     */
//    public void pipelineSet(Map<String,Object> map, Long seconds){
...
//                byte[] key = keySerializer.serialize(next.getKey());
//                connection.set(key,
